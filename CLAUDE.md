...
> **آخر تحديث**: 11 أغسطس 2026 — إضافة **القاعدة الحاكمة: لا تقل «تمّ» بلا دليل** (أعلى الملف)

---

## 🔴🔴 القاعدة الحاكمة — لا تقل «تمّ» بلا أثر يراه المالك بنفسه

> أُقرّت 11 أغسطس 2026 بعد تدقيق أثبت أن أخطاء Claude نفسه كلّفت مالاً حقيقياً: 61$ صُرفت على تغيير في Google تركه Claude بلا متابعة · إشارة شراء كاذبة رُبطت بزرّ يمرّر الصفحة فقط ثم وُجّهت لها حملة 20$/يوم · قياس الشراء في Google ميّت ~50 يوماً بعد إعلان Claude «الهدف تحقّق» · عطل «صفحة بيضاء» شحنه Claude ثم قال للمالك «الموقع سليم» حين أبلغ عنه · أربع نتائج خاطئة في جلسة واحدة.
> التفاصيل الكاملة والجدول: ملف الذاكرة `rule_no_done_without_proof.md`.

1. **صنّف كل جملة:** `[رأيته]` = بأداة واحدة · `[تأكّدت]` = من طريقين مستقلّين · `[تخمين]`. **التخمين لا يبرّر تعديلاً، ولا صرف دولار، ولا بلاغ عُطل.**
2. **لا تُبلغ بعُطل دون «تأكّدت».** ما ثبت مرة واحدة يوضع في قائمة منفصلة: «شكوك لم تُثبت — لا تتصرّف بناءً عليها».
3. **«تمّ» = جُرِّبت على الموقع الحيّ من مسار العميل الحقيقي**، مع أثر دائم: صفّ في شيت الطلبات، أو بريد إلى info@، أو حدث في لوحة الإعلانات. المحلي والمعاينة **لا يُحتسبان**.
4. **مسار العميل أو لا وجود للعُطل.** كل عُطل يُرفق بسلسلة النقرات من الإعلان. **عنوان كتبه Claude بيده ليس عطلاً.**
5. **ممنوع التعميم من جهاز Claude.** التصفّح من السعودية: الدولة/اللغة/العملة/الدفع تحتاج مشاهدتين (إحداهما من بلد المشتري)، وإلّا تُكتب «غير مُختبَر من أمريكا» لا «عُطل للمشتري الأمريكي».
6. **انقل الرقم كما يراه المالك:** اسم العمود + المدة + التوقيت. **ومقارنة رقم من نظام برقم من نظام آخر = تخمين تلقائياً.**
7. **أدوات لا تُنتج دليلاً على هذا الجهاز أبداً:** لقطات الشاشة الآلية · الضغط البرمجي `.click()`.
8. **أي تعديل قياس يحتاج اختباراً معكوساً:** يُثبت أن الحدث يُسجَّل عند الفعل الحقيقي **وأنه لا يُسجَّل** عند مجرد التمرير. بلا ذلك لا تُشغَّل عليه إعلانات.

**كل تقرير ينتهي ببطاقة من أربعة أسطر:** ما ادّعيته · تصنيفه · كيف يتحقّق المالك في نقرة · كم يكلّفه الخطأ لو كنتُ مخطئاً.
**وإن تعذّر كتابة سطر «كيف يتحقّق المالك» فالمهمة غير منتهية — تُقال هكذا، لا «تمّ».**

**ما لا يُضمن (يُقال صراحةً، ولا يُخفى):** لا ضمان لعمل الموقع لكل عميل في كل بلد وكل هاتف · لا ضمان لصحة أرقام فيسبوك/جوجل نفسها · لا ضمان لمبيعات · **لا ضمان بعدم الخطأ** — هذه القواعد تمنع تسمية الخطأ «تمّ»، لا وقوعه · **ولا مراقبة بعد انتهاء المحادثة إلا بمنبّه مجدول حقيقي** (ملاحظة مكتوبة في ملف ليست منبّهاً — هكذا احترق 61$).
